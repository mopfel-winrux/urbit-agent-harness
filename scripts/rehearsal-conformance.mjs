...
const server = createServer(async (req, res) => {
  try {
    ...
    if (child) {
      childTurns++
      assert.deepEqual(body.tools.map((t) => t.function.name).sort(), ['calculate', 'current_time', 'lcm_expand', 'lcm_read', 'lcm_search', 'list_desk_files', 'list_desk_scopes', 'read_desk_file', 'read_skill'])
      if (!receipts.length) {
