...
++  fixture
  ...
  saved
++  invoke
  |=  [saved=state-21 method=@t params=json]
  ^-  [(list card:agent:gall) state-21]
  ...
  ?>  ?=(%0 -.out)
  ;;([(list card:agent:gall) state-21] product.out)
++  invoke-raw
  |=  [saved=state-21 method=@t params=json]
  ^-  [(list card:agent:gall) state-21]
  ...
  [-.out !<(state-21 ~(on-save +.out bowl))]
++  reset-params
  |=  [saved=state-21 ship=@p]
...
++  delete-fixture
  ...
  =/  next  !<(state-21 ~(on-save +.out bowl))
