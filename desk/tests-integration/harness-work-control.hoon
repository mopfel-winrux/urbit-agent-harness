...
++  test-human-help-and-empty-argument-reads-need-no-model
  ...
  =/  after  !<(state-25 ~(on-save +.out bowl))
  =/  rows
...
++  test-owner-command-prepares-then-confirms-once-without-model-work
  ...
  =/  before  !<(state-25 ~(on-save +.prepared bowl))
...
++  test-reviewed-reply-reload-retains-one-literal-effect-and-exclusive-claim
  ...
  =/  id  p.i.rows
  ?>  =(~ outbox.hands.before)
  =/  sent  (~(on-poke +.prepared bowl) %harness-action !>(`action:h`[%send 'owner' (cat 3 '/work confirm ' (scot %uv id))]))
  =/  queued  !<(state-25 ~(on-save +.sent bowl))
  ...
  =/  restored  !<(state-25 ~(on-save +.reloaded bowl))
  ...
  =/  once  !<(state-25 ~(on-save +.duplicate bowl))
  ?>  =(hands.queued hands.once)
  =/  claim  (~(on-poke +.duplicate bowl) %harness-hand !>(`request:hh`['claim' [%claim 'fixture' effect 'publisher']]))
  =/  claimed  !<(state-25 ~(on-save +.claim bowl))
  ...
  ;:  weld
    ...
    (expect-eq !>(workspace.before) !>(workspace.final))
  ==
--
