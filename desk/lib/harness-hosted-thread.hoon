...
++  call
  ...
  =?  p.args  &(!=(action 'settings') ?~((optional:j args 'requestId') & |))
    (~(put by p.args) 'requestId' [%s id])
  ;<  ~  bind:m  (watch:io /response [our %harness] /hosted/[id])
  ;<  ~  bind:m  (poke:io [our %harness] %harness-hosted !>(`request:hosted`[id action args]))
