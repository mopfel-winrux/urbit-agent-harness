...
+$  state-0
  $:  %0
      ...
      ::  per-session run_js CPU-time bound; absent = the default, 0 = no limit
      js-timeouts=(map session-id:h @dr)
  ==
::  state-le: the pre-js-timeouts shape, used only to migrate old state.
+$  state-le
  $:  %0
      model-defaults-set=$~(| ?)
      xai-auth=state:oauth
