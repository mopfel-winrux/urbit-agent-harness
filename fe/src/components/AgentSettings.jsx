...
export default function AgentSettings({ resources, theme, onThemeChange }) {
  ...
  return <form className="settings-grid" onSubmit={save}>
    ...
    <section className="panel settings-panel">
      ...
      <label><span>JavaScript execution timeout (seconds)</span><input type="number" min="0" step="1" value={form['js-timeout'] ?? 30} onChange={(event) => field('js-timeout', Math.max(0, Math.floor(Number(event.target.value) || 0)))} /></label>
      <p className="field-note">CPU-time limit for each <code>run_js</code> call. A script that never finishes — including a tight infinite loop — is stopped after this many seconds instead of blocking the ship. <strong>0 means no limit</strong> (a runaway script can then stall the ship).</p>
    </section>
    <section className="panel settings-panel">
