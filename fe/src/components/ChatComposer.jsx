...
export default function ChatComposer({ busy, loading, usage, compactions, onSend, onStop }) {
  ...
  return <div className="composer-wrap"><form className="composer" onSubmit={send}>
    ...
    {busy ? <button type="button" className="composer-button stop" onClick={onStop} title="Interrupt" aria-label="Interrupt"><StopIcon /></button>
      : <button className="composer-button" disabled={!draft.trim() || loading} title="Send" aria-label="Send"><SendIcon /></button>}
  </form><small>Enter to send · Shift+Enter for a new line{usage?.completion > 0 && ` · ${usage.completion.toLocaleString()} output tokens`}{compactions > 0 && ` · ${compactions} context summaries`}</small></div>
}
