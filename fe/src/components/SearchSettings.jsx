...
export default function SearchSettings() {
  ...
  return <form className="settings-grid" onSubmit={saveProvider}>
    {(error || status.error || stored.error) && <div className="inline-error">{error || status.error || stored.error}</div>}
    ...
    <div className="save-bar"><span>{saved ? 'Saved.' : 'Each conversation controls its Web permission.'}</span><button className="button primary" disabled={busy}>{busy ? 'Saving…' : 'Save search provider'}</button></div>
  </form>
}
