...
const address = (record) => `${record.scope}:${record.eventCount}`
const provenance = (record) => [record.hand, record.author, record.kind, record.sent ? new Date(record.sent).toLocaleString() : 'Date unavailable'].filter(Boolean).join(' · ')
...
export default function CorpusSearch({ onBack, onOpen }) {
  const status = useResource('corpus/status', null, 2500)
  ...
  async function search(event, more = false, retryRequest = null) {
    ...
    try {
      const next = await api.corpus('search', { query: text, limit: 20, ...(more ? { cursor } : {}) })
  ...
  return <main className="workspace corpus-workspace">
    ...
    <div className="corpus-content">
      <div className="page-header"><h1>Search content</h1><p>Find retained messages, tool results, and summaries across every Harness conversation and hand.</p></div>
      <form className="corpus-search-form" role="search" onSubmit={search}>
        <label><span>Search the Harness corpus</span><input type="search" autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Words from a message or decision" required /></label>
        <button className="button primary" disabled={busy || !query.trim()}>{busy ? 'Searching…' : 'Search'}</button>
      </form>
      <p className="field-note corpus-status" role="status">{status.value ? `${status.value.indexed.toLocaleString()} records · ${status.value.conversations.toLocaleString()} conversations${status.value.indexing ? ' · Indexing retained history; results may be incomplete.' : ' · Index up to date.'}` : status.loading ? 'Loading index status…' : 'Index status unavailable.'}</p>
      ...
      <div className={`corpus-layout${selected ? ' has-source' : ''}`}>
        <section className="corpus-results" aria-label="Search results" aria-busy={busy}>
          {!submitted && <div className="corpus-empty"><h2>Search what Harness has encountered</h2><p>Includes content received from Tlon and other hands, plus conversation messages and retained summary evidence. It does not search content Harness has never received.</p></div>}
          ...
          {result?.hits.filter(Boolean).map((record) => <div key={address(record)}>
            <button ref={(element) => { if (element) resultButtons.current.set(address(record), element); else resultButtons.current.delete(address(record)) }} className={`corpus-hit${trail[0] && address(record) === address(trail[0]) ? ' active' : ''}`} onClick={() => select(record)} aria-pressed={!!trail[0] && address(record) === address(trail[0])}>
              <strong>{record.sessionId}</strong><span className="corpus-hit-meta">{provenance(record)}</span><span className="corpus-hit-snippet">{record.snippet}</span>
            </button>
          </div>)}
        ...
        {selected && <section className="corpus-detail panel" ref={detail} tabIndex={-1} aria-label="Selected source">
          ...
          <Source key={address(selected)} record={selected} onSelect={(record) => select(record, true)} onOpen={onOpen} />
        </section>}
      </div>
    </div>
  </main>
