...
const metadata = (artifact) => { const { revisions, ...rest } = artifact; return clone(rest) }
...
acp.start = async () => {}
acp.ship = () => 'lux'
const writes = new Set(['artifact-create', 'artifact-save', 'artifact-archive', 'project-create', 'project-edit', 'member', 'publish', 'unpublish', 'review', 'task-create', 'task-claim', 'task-update'])
...
acp.call = async (method, { action, args = {} }) => {
  ...
  if (action === 'artifact-create') {
    ...
    if (args.confirm !== `${args.id}@${args.revision}` || args.head !== artifact.head || args.exposure !== artifact.exposure) throw new Error('Publication changed; preview it again.')
    artifact.exposure++; artifact.publication = { revision: args.revision, slug: args.slug, path: `/harness-pages/${args.slug}`, at }; result = { artifact: metadata(artifact) }
...
function Fixture() {
  ...
  return <div className="app-shell"><Sidebar chats={['neighborhood-research', 'afternoon-plans']} current="" onSelect={() => {}} onNew={() => {}} onSettings={() => {}} onWorkspace={(kind) => { location.hash = `#/${kind}` }} work={view.kind} /><Workspace kind={view.kind} id={view.id} onBack={() => { location.hash = '#/artifacts' }} /></div>
}
